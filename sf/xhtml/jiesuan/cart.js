...
function changeCart(obj){
  ...
  $.ajax({
    ...
    beforeSend: function(){
      ...
      $('#'+temp[0]+'load').html('<img src="'+SF_STATIC_URL+'/com/images/loading32.gif">').show();
      $('#'+temp[0]).hide();
    },
          ...
          for(var t in msg.data){
            if($('#'+t).length > 0){
              ...
              if (msg.data[t] != 'none') {
                ...
                $("#all_putong > div.cartOrderTotal").before(loadDiv);
                $("#all_putong > div.cartOrderTotal").before("<div id='"+t+"'>"+msg.data[t]+"</div>");
              }
            }
          }
        }
...
function changNum(num,value){
  ...
  changeCartNum(value,count);
}

function changCount(value,obj){
  var selceobj = $("input[type='checkbox'][name='cart_list'][value='"+value+"']:not(:checked)");
  if(selceobj.length > 0){
    selceobj.attr("checked",true);
    changeCart(selceobj);
...
//点击商品数量
function changeCartNum(value,count,bs){
  if (typeof(bs) == "undefined") { 
    bs = 1; 
  }
  var hasDiv = new Array();
  $("#all_putong").children().each(function(k, v){
    var divId = $(v).attr('id');
  ...
  $.ajax({
    ...
    type : 'POST',
    data : {val:value, num:count, mes:bs, hasdiv:hasDiv},
    dataType: 'json',
    success: function(msg){
      //$('.currcart').each(function(){
      //	$(this).removeClass("currcart");
      ...
      if(msg.error==1){
        ...
        if (msg.info.type.length > 0) {
          for(var t in msg.data){
            ...
            if (msg.data[t] != 'none') {
              var loadDiv = $("<div style='margin:20px;text-align:center;display:none;' id='"+t+"load'></div>");
              $("#all_putong").prepend(loadDiv);
              $("#all_putong").prepend("<div id='"+t+"'>"+msg.data[t]+"</div>");
            }
          }
        }
        chmodeNum(msg.info.cart);
...
function cartJump()
{
  if($("input[name='cart_list']:checked").length==0){
    jAlert("请至少选择一个商品");	
    return;
  }
